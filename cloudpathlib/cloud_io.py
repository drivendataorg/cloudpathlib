...
class _CloudStorageRaw(io.RawIOBase):
    ...
    def readinto(self, b: bytearray) -> int:  # type: ignore[override]
        ...
        try:
            ...
            # Fall back to memoryview-based copy for non-contiguous buffer shapes
            memoryview(b).cast("B")[:n] = data[:n]
    ...
    def close(self) -> None:
        ...
        # Mark as closed FIRST to prevent recursive double-finalize
        self._closed = True
...
class CloudBufferedIO(io.BufferedIOBase):
    ...
    def close(self):
        # Delegate entirely to the stdlib buffer: flush → raw.close().
        # _CloudStorageRaw.close() guards against double-finalize via _closed,
        # so calling self._buffer.close() is safe and lets exceptions propagate.
